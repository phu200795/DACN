...
$sl="select * from loai_con where loaicon_id={$_GET["loaicon_id"]} ";
$kq=mysql_query($sl);
...
$url = 'https://fptshop.com.vn/dien-thoai/'.$parent2.'?sort=gia-cao-den-thap';
...
function save_all_sanpham($loaicon_id,$html){
 $post = array();
    ...
    $newsItems = $block->find('.fs-lpil');
    if(isset($newsItems)){

      foreach ($newsItems as $key => $value) {
        $name_img =$value->find('.fs-lpil-img')[0];
        $name = $name_img->getAttribute('title');
        $i = $name_img->find('img[class=lazy]')[0];
        $im=$i->getAttribute('data-original');
        $price=$value->find('.fs-lpil-if')[0]->find('.fs-lpilname')[0]->find('.fs-lpil-price')[0]->text();
