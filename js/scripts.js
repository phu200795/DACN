...
$(document).ready(function(){
	$('.group-link-collection li').click(function(){
		...
		if ( $(this).attr('data-loaded') == 'false' && $(this).attr('data-handle') == '' ) {
			$(this).attr('data-loaded', 'true');
			$(this).parents('.box-section-collection').find('.group-collection .box-product-lists').eq($(this).index()).append("<div class='clearfixProduct empty'><p>KhĂ´ng tĂ¬m tháº¥y káº¿t quáº£. Vui lĂ²ng thá»­ láº¡i !</p></div>");
		}
...
$(document).ready(function(){
	...
	function getCartView() {
		jQuery.getJSON('/cart.js', function(cart, textStatus) {
			...
			$('#view-cart').append(
				"<div id='view-h' class='cart-list' ></div><div class='text-mini-cart'><span class='cart_block_text'>Tá»•ng tiá»n:</span><span class='cart_block_total'>" + Haravan.formatMoney(cart.total_price,'') +"Ä‘" + "</span></div><div class='checkout cart-check-mini'><a class='button btn view-cart' href='/cart'><span>Giá» hĂ ng</span></a><a class='button btn checkout' href='/checkout'><span>Thanh toĂ¡n</span></a></div>"
			);
		...
		var params = {
			...
			success: function(cart) {  
				if ( cart.item_count > 0 ) {
					...
					if ( window.location.pathname == '/cart' ){
						$('#cartformpage').remove();
						$('#layout-page').append("<p class='text-center'>KhĂ´ng cĂ³ sáº£n pháº©m nĂ o trong giá» hĂ ng!</p><p class='text-center'><a href='/collections/all'><i class='fa fa-reply'></i> Tiáº¿p tá»¥c mua hĂ ng</a></p>");
					}
					$('.ajax_cart_quantity').html(cart.item_count);
					$('#view-cart > div:not(#clone-item)').remove();
					$('#view-cart').append("<div style='padding:10px 20px;'> <p style='margin:0' class='text-center'>Giá» hĂ ng cá»§a báº¡n Ä‘ang trá»‘ng</p><p class='text-center'><a href=''>Tiáº¿p tá»¥c mua hĂ ng</a></p></div>");
				}
			},
	...
	Haravan.onItemAdded = function(line_item) {
		...
		jQuery.getJSON('/cart.js', function(cart, textStatus) {
			if(cart)
			{
				...
				$('.item-total').html(Haravan.formatMoney(cart.total_price, "")+"Ä‘"); 
				$('.layer_cart_cart.first .item-price').html(Haravan.formatMoney(line_item.price, "")+"Ä‘");
				$('.cart-count').html(cart.item_count);
				$('.layer_cart_img').attr('href', line_item.url);
				$('.item-quantity').html('<strong>Sá»‘ lÆ°á»£ng:</strong>'+line_item.quantity);
			}
		})
	}
	/*-------------------------------------*/
				 ...
				 jQuery.getJSON('/cart.js', function(cart, textStatus) {
					 if(cart)
					 {
						 ...
						 $.each(cart.items,function(i,item){
							 ...
							 tr.find('.item-delete').html("<a href='#' onclick='deleteCart(" + item.variant_id + ")' >XĂ³a</a>");
						 });
						 if ( typeof(formatMoney) != 'undefined' ){
						 ...
						 if(cart.item_count == 0){
							 ...
							 $('#myCart #exampleModalLabel').html('Giá» hĂ ng cá»§a báº¡n Ä‘ang trá»‘ng. Má»i báº¡n tiáº¿p tá»¥c mua hĂ ng.');
						 }
						 else{
							 $('#myCart #exampleModalLabel').html('Báº¡n cĂ³ ' + cart.item_count + ' sáº£n pháº©m trong giá» hĂ ng.');
				 ...
				 $('#update-cart-modal').click(function(event){
					 ...
					 var params = {
						 ...
						 success: function(cart) {
							 ...
							 $('#update-cart-modal').html('Cáº­p nháº­t');
						 },
						 error: function(XMLHttpRequest, textStatus) {
							 Haravan.onError(XMLHttpRequest, textStatus);
						 }
