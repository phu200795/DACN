...
<div id="main-content" class="product-content page_commerce">
    <h1 class="title clearfix"><span>Thời trang</span></h1>
        ...
        <div class="browse-tags pull-left">
            ...
            <span class="custom-dropdown custom-dropdown--white">
                <select id="lblimit" name="lblimit" class="sort-by custom-dropdown__select custom-dropdown__select--white" onchange="window.location.href = this.options[this.selectedIndex].value">
                    ...
                    ?>
                            <option <?php if(isset($_GET['lblimit'])&&$_GET['lblimit']==$a[$i]){echo "selected='selected'";$idTL=$_GET['idTL'];}?> value="?limit=<?php echo $a[$i]?>&idloaicha=<?php echo $_GET['idloaicha']?>"><?php echo $a[$i]?></option>
                            <?php }?>
    ...
    <div class="product_list grid row products-grid content-product-list products">
        ...
        <?php
        $sqlc="select * from loai_con where loaicha_id={$_GET['idloaicha']}";
        $kqc=mysql_query($sqlc);
        $dc=mysql_fetch_array($kqc);
       if(!isset($_GET['idloaicon'])){
        if(isset($_GET['limit'])){
            $sql="select * from san_pham where loaicon_id={$dc['loaicon_id']}   order by rand() limit 0,{$_GET['limit']}";
        }
        else{
            $sql="select * from san_pham where loaicon_id={$dc['loaicon_id']}   order by rand() limit 0,12";
        }
        $kq=mysql_query($sql);
        ...
        ?>
          ...
          <?php 
            ...
            if(isset($_GET['limit']))
             $sql="select * from san_pham where loaicon_id={$_GET['idloaicon']}   order by rand() limit 0,{$_GET['limit']}";
            else
                $sql="select * from san_pham where loaicon_id={$_GET['idloaicon']}   order by rand() limit 0,12";
...
<div class="menu-product ts-product-categories-widget">
    <div type="button" class="collapsed" data-toggle="collapse" data-target="#category_menu" aria-expanded="false">
        ...
        <span class="sr-only">Toggle navigation</span>
         <?php
        $sqlc="select * from loai_cha where loaicha_id={$_GET['idloaicha']}";
        $kqc=mysql_query($sqlc);
        $dc=mysql_fetch_array($kqc);
        $sql="select * from loai_con where loaicha_id={$dc['loaicha_id']} ";
        $kq=mysql_query($sql);
       
        ?>
        <div class="title-wrapper">
            <h3 class="hr_title heading-title"><a href="xem-sp-dienthoai.php?idloaicha=<?php echo $dsp['loaicha_id'] ?>" title=""><?php echo $dc['loaicha_name'] ?></a></h3>
        </div>
    </div>
    <div class="block_content navbar-collapse collapse" aria-expanded="false" id="category_menu">
        ...
        <ul class='list-block product-categories tree dhtml'>
            <?php
             while($d=mysql_fetch_array($kq))
        {?>
<li><a href='xem-sp-dienthoai.php?idloaicha=<?php echo $_GET['idloaicha']?>&idloaicon=<?php echo $d['loaicon_id']?>'><span class='menu-icon'><i class='fa fa-check'></i></span> <span class='menu-label'><?php echo $d['loaicon_name'] ?></span></a></li>
