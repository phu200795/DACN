...
$url = 'https://fptshop.com.vn/dien-thoai/apple-iphone?sort=gia-cao-den-thap';

if(get_data($url, $content)){
    $d_cha=save_all_loaicha($content);
    $loaicha_id=$d_cha['loaicha_id'];
    $d_con=save_all_loaicon($loaicha_id,$content);
...
function save_all_loaicon($loaicha_id,$html){
    $dom = new Document();
    $dom->load($html);
...
function insert_loaicon($loaicha_id,$con){
...
if(get_data($urll, $content)){
    $d=save_all_loaicha_laptop($content);
    $loaicha_id_laptop=$d['loaicha_id'];
    save_all_loaicon_laptop($loaicha_id_laptop,$content);
...
function save_all_loaicha_laptop($html){
    ...
    $block = $dom->find('.fs-ctf-head')[0];

    if(isset($block)){
      $newsItems = $block->find('.fs-ctf-hl')[0];
      if(isset($newsItems)){
        $name = $newsItems->find('a')[0]->text();
...
function save_all_loaicon_laptop($loaicha_id,$html){
    ...
    if(isset($newsItems) && count($newsItems)>0){
      for($i = 1 ;$i <=7 ;$i++){
        ...
        $d=insert_loaicon_laptop($loaicha_id,$post);
